...
data1 <- data.frame(gr=gr, value=value)

m_sim1 <- brm(value~ 0 + gr, data=data1, family = lognormal(), file = "m_sim1")
m_sim2 <- brm(value~ 0 + (1|gr), data=data1, family = lognormal(), file = "m_sim2")
m_sim3 <- brm(bf(value~ 0 + (1|gr), sigma~ 0 + (1|gr)), 
              data=data1, family = lognormal(), file = "m_sim3")
m_sim4 <- brm(bf(value~ 0 + (1|id|gr), sigma~ 0 + (1|id|gr)), 
              data=data1, family = lognormal(), file = "m_sim4")
```
...
```{r}
m_sim2p <- posterior_summary(m_sim2) %>% exp() %>% as.data.frame() %>% rownames_to_column() %>% filter(str_detect(rowname, "r_gr") ) 
m_sim2p$rowname <- m_sim2p$rowname %>%  str_replace_all("r_gr\\[", "")
m_sim2p$rowname <- m_sim2p$rowname %>%  str_replace_all(",Intercept\\]", "")
m_sim2p$index <- "m_sim2"

m_sim1p <- posterior_summary(m_sim1) %>% as.data.frame()%>% exp() %>% rownames_to_column() %>% filter(str_detect(rowname, "b_gr") )
m_sim1p$rowname <- m_sim1p$rowname %>%  str_replace_all("b_gr", "")
...
```{r}
ggplot(m_sim, aes(rowname, Estimate, color=index)) + geom_pointinterval(aes(ymin=`Q2.5`, ymax=`Q97.5`), position = "dodge")+ xlab(NULL)+ ylab("median PT hrs")+ coord_flip()
```
...
```{r}
m1 <- brm(data=HF_data, postacute_therapy_binned~1, family = bernoulli(), file = "m1")
m1l <- loo(m1)

m2 <- brm(data=HF_data, postacute_therapy_binned~0 + county, family = bernoulli(), file = "m2")
m2l <- loo(m2)

m3 <- brm(data=HF_data, postacute_therapy_binned ~ 0 + (1|county), family = bernoulli(), file = "m3")
m3l <- loo(m3)
```
...
```{r}
(m_anova_pt <- brm(data= HF_data %>% filter(postacute_therapy>0),
          postacute_therapy~0 + (1|county), family = lognormal(), file = "m_anova_pt"))
```
