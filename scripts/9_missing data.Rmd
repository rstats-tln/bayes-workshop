...
viss_miss comes from **naniar** package.
...
```{r}
get_prior(scv ~ tp * age + (1| subj), data = wrist, family = gaussian())
```
...
Another model with age classes
```{r}
fit0.1 <- brm(orf1ab_ct ~ lineage + symptomatic + age10, 
            data = data, 
            family = gaussian(), 
            prior = prior("normal(0, 2.5)", class = "b"),
            file = here("models/orf1ab_ct ~ lineage + symptomatic + age10"),
            chains = 2)
```
...
```{r}
plot(conditional_effects(fit0.1, effects = "age10"))
```
...
age <- bf(age | mi() ~ sex + symptomatic)
fit1 <- brm(formula = ct + age + set_rescor(FALSE), 
            data = data1, 
            family = gaussian(), 
            prior = prior("normal(0, 2.5)", class = "b"),
            file = here("models/bf(orf1ab_ct | mi() ~ lineage + symptomatic + mi(age)) + bf(age | mi() ~ sex + symptomatic)"),
            chains = 2)
```
...
```{r}
fit <- brm(
  bf(mvbind(scv, amp, sdl) ~ tp * age + (1 | subj)) + set_rescor(TRUE), 
  data = wdata, 
  family = gaussian(), 
  chains = 2,
...
```{r}
scv <- bf(scv ~ tp * age + (1 |i| subj))
amp <- bf(amp ~ tp * age + (1 |i| subj))
sdl <- bf(sdl ~ tp * age + (1 |i| subj)) + skew_normal()
fit3 <- brm(
