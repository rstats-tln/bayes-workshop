...
b <- rnorm(3000, median(samples2$b_Intercept), median(samples2$sigma))
sd(a)
sd(b)
```
...
Use strong informative prior for sigma *normal(7.4, 1)*

Set seed for reproducibility!
```{r}
set.seed(20211002)
N <- 3
data <- sample_n(president_heights, N)
```
...
priors <- c(
  prior("normal(178, 20)", class = "Intercept"),
  prior("normal(7.4, 1)", class = "sigma")
  )
mod <- brm(
  formula = f, 
  data = data, 
